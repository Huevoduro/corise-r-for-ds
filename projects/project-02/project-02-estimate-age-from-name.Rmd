...
tbl_lifetables |> 
  ...
  labs(
    title = "Probability of being Alive per Year",
    subtitle = "by sex",
    x = "Year",
    y = "Probability of being alive",
    caption = "Source: plagiarism ;P"
  ) +
...
plot_estimated_age <- function(tbl, my_name, my_sex) {
  tbl |> 
    ...
    labs(
      ...
      subtitle = "Number of births and alive people",
      x = "Year",
      y = "Count",
    ) +
...
tbl_names_female_top_25 <- tbl_names |>
  ...
  # select(!nb_births) |> # this line is giving me problems and doesn't add any value
  # Filter only Females
  filter(sex == "F") |>
  # Slice top 25 rows
  slice_max(nb_births_total, n = 25) |>
  # Select name and sex
  select(name, sex)
